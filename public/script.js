...
// Initialize once the page markup exists
function initializePortfolioSite() {
...
function updateActiveNavLink() {
    ...
    function normalizeNavHref(value) {
        const hrefPath = (value || '').split(/[?#]/)[0].replace(/^\//, '') || 'home';
        return prettyMap[hrefPath] || hrefPath;
    }
...
function createScrollToTopButton() {
    ...
    button.innerHTML = '<svg class="lucide-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="m18 15-6-6-6 6"></path></svg>';
...
function initializeCarousels() {
    ...
    // Client Logos Carousel - 2.2 second interval
    initializeCarousel('.clients-carousel', '.client-prev', '.client-next', '.client-indicator', 2200);
}
...
function renderProjectDetailPage() {
    const isProjectRoute = window.location.pathname.replace(/\/$/, '') === '/project';
    if (!document.body || (document.body.getAttribute('data-page') !== 'project-detail' && !isProjectRoute)) return;
    ...
    if (!data) {
        ...
        setTimeout(() => { window.location.replace('/projects'); }, 600);
        return;
    }
...
console.log('%cWelcome to my portfolio!', 'font-size: 20px; color: #16c784; font-weight: bold;');
...
const THEME_ICONS = {
    sun: `
        <svg class="theme-icon-svg" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
    ...
    moon: `
        <svg class="theme-icon-svg" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
            <path d="M20.99 12.62A8.5 8.5 0 1 1 11.38 3.01 6.5 6.5 0 1 0 20.99 12.62Z"></path>
        </svg>`
};
